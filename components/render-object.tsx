...
export default function RenderObject({ title, obj, className, style, key, keys = Object.keys(obj || {}) }: Props) {
  if (!obj) {
    return null;
  }
      ...
      {obj &&
        keys.map((k, i) => {
          ...
          if (Array.isArray(val)) {
            ...
            return <RenderObject key={i.toString()} obj={val} className={className} />;
          } else if (typeof val === "string" && val.indexOf("+00:00") > -1) {
            return (
              <div className="render-object-key" key={i}>
                {convertCamelToHuman(k)}: <b>{formatDate(val, true)}</b>
              </div>
            );
          }
